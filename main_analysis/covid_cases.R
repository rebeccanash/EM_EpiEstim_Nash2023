...
daily_inc <- readRDS("real_data/daily_covid_cases.rds")
ndays <- length(daily_inc$Incidence)
index <- seq(1, ndays)

# Aggregate to weekly
weekly_inc <- EpiEstim::aggregate_inc(daily_inc$Incidence, dt = 7L)
saveRDS(weekly_inc, "supplementary_analysis/covid_cases/weekly_inc_covid.rds")
...
saveRDS(r_rep_sliding, "supplementary_analysis/covid_cases/r_rep_sliding_result.rds")
...
r_agg_sliding <- EpiEstim::estimate_R(incid = weekly_inc,
                                      ...
                                      method = method,
                                      config = config)
saveRDS(r_agg_sliding, "supplementary_analysis/covid_cases/r_agg_sliding_result.rds")

# Estimate R from reported daily incidence (non sliding - daily R estimate)
r_rep <- EpiEstim::estimate_R(incid = daily_inc$Incidence,
                              method = method,
                              config = config_daily)
saveRDS(r_rep, "supplementary_analysis/covid_cases/r_rep_result.rds")
...
r_agg <- EpiEstim::estimate_R(incid = weekly_inc,
                              ...
                              method = method,
                              config = config)
saveRDS(r_agg, "supplementary_analysis/covid_cases/r_agg_result.rds")
...
plot_inc <- ggplot(inc_dat, aes(x = index)) +
  ...
  ylab("Daily incidence by date of specimen") +
...
corr1 <- ggplot(scatter_dat1, aes(x = rep, y = agg)) +
  ...
  annotate("text", x = 2.1, y = 1.4, label = "R^2==0.98", size = 4,
           parse = TRUE) +
...
uncertainty1 <- ggplot(scatter_dat1, aes(x = width_daily, y = width_weekly)) +
  ...
  xlim(0, 0.125) +
  ylim(0, 0.125) +
  annotate("text", x = 0.1025, y = 0.03, label  = "R^2==0.999", size = 4,
           parse = TRUE) +
...
corr2 <- ggplot(scatter_dat2, aes(x = rep, y = agg)) +
  ...
  scale_colour_manual("",
                      breaks = c("early", "mid", "late"),
                      values = c("#B3E5FC", "dodgerblue", "#01579B")) +
  annotate("text", x = 2.9, y = 1.6, label = "R^2==0.67", size = 4,
           parse = TRUE) +
...
uncertainty2 <- ggplot(scatter_dat2, aes(x = width_daily, y = width_weekly)) +
  ...
  xlim(0, 0.18) +
  ylim(0, 0.18) +
  annotate("text", x = 0.15, y = 0.045, label = "R^2==0.97", size = 4,
           parse = TRUE) +
...
p1 <- plot_inc + coord_cartesian(xlim = c(0, 679))
p2 <- squared_err + coord_cartesian(xlim = c(0, 679), ylim = c(0, 0.5))

cov_plot <- plot_grid(p1, corr1, corr2, p2, uncertainty1, uncertainty2,
                      ncol = 3, align = "hv",
                      labels = c("A", "C", "E", "B", "D", "F"),
                      label_size = 12, rel_widths = c(1.25, 1, 1))

ggsave(plot = cov_plot, "figures/Figure_3.pdf", device = cairo_pdf,
       width = 12, height = 7)
