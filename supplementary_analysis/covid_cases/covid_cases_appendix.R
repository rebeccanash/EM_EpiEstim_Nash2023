## UK COVID cases: Supplementary analysis
...
weekly_inc <- readRDS("supplementary_analysis/covid_cases/weekly_inc_covid.rds")
r_rep <- readRDS("supplementary_analysis/covid_cases/r_rep_result.rds")
r_agg <- readRDS("supplementary_analysis/covid_cases/r_agg_result.rds")
r_rep_sliding <- readRDS("supplementary_analysis/covid_cases/r_rep_sliding_result.rds")
r_agg_sliding <- readRDS("supplementary_analysis/covid_cases/r_agg_sliding_result.rds")
...
r_plot2 <- ggplot(dat2, aes(x = day, color = source,
  ...
  scale_color_manual("",
                     ...
                     labels = c("Reported data",
                                "Reconstructed data")) +
  scale_fill_manual("",
                    values = c("rep_dat_s" = alpha("grey", 0.),
...
ggsave(plot = r_plots_covid, "supplementary_figures/covid_cases_rt.pdf",
...
panel_a <- plot_inc + coord_cartesian(xlim = c(1, 679))
panel_b <- plot_log_inc + coord_cartesian(xlim = c(1, 679))
panel_c <- diff_plot1 + coord_cartesian(xlim = c(1, 679))
panel_d <- diff_plot2 + coord_cartesian(xlim = c(1, 679))
cov_bias <- plot_grid(panel_a, panel_b, panel_c, panel_d, ncol = 1, align = "v", 
                      labels=c("A", "B", "C", "D"), label_size = 12)

ggsave(plot = cov_bias, "supplementary_figures/covid_rt_bias.pdf",
