## Simulation study: time-varying Rt with gradual change
...
# Simulated incidence for gradual increase/decrease in Rt (simulate_incidence.R):
sims <- list()
...
#           file = paste0("supplementary_analysis/tv_rt_gradual/result_rt_grad_",
#                         names(sims[x]), ".rds"))
# }
# 
...
for (n in seq_along(all_names)) {
  result[[n]] <- readRDS(
    paste0("supplementary_analysis/tv_rt_gradual/result_rt_grad_",
           all_names[n], ".rds"))
}
...
r_plot <- function(data, legend_pos = "none", ymin = 0.5, ymax = 2,
                   xmin = 14, xmax = 70, legend_size = NA) {
...
for (res in seq_along(names)) {
  true <- c(rep(true_all[[res]][1], 5),
            seq(from = true_all[[res]][1],
                to = true_all[[res]][2], length = 32),
            rep(true_all[[res]][2], 20))
...
bias_plot <- function(data) {
  ggplot(data, aes(x = t_end, y = bias)) +
    geom_vline(xintercept = 20, lty = 2, col = "lightgrey") +
    geom_vline(xintercept = 50, lty = 2, col = "lightgrey") +
...
for (res in seq_along(names)) {
  for (i in seq_along(sim_idx)) {
    ...
    cov_mx[[res + n_results]][, i] <-
      process_cov(dat = ref_mx[[res]]$true_r,
                  ests = get(paste0("ests", res))[[i]])
  }
}
...
round(perc_time[1:3], 2) # increasing [77 - 96%]
round(perc_time[4:7], 2) # decreasing [61 - 94%]
...
ggsave(plot = dec_plots, "supplementary_figures/tv_rt_gradual_decrease.pdf",
       device = cairo_pdf, width = 12, height = 12)

ggsave(plot = inc_plots, "supplementary_figures/tv_rt_gradual_increase.pdf",
       device = cairo_pdf, width = 12, height = 12)
...
for (res in seq_along(names)) {
  true <- c(rep(true_all[[res]][1], 8),
            seq(from = true_all[[res]][1],
                to = true_all[[res]][2], length = 32),
            rep(true_all[[res]][2], 17))
...
for (l in seq_along(increasing)) {
  plot_r <- r_plot(data = get(paste0("mid_dat", increasing[l])),
                   legend_pos = "none", ymin = 0.75, ymax = 1.8,
                   xmin = 10, xmax = 70)
...
mid_perc_time <- mid_cov_dat %>% select(starts_with("mid_cov_t")) %>%
  summarise(across(where(is.numeric), mean))
round(mid_perc_time[1:3], 2) # increasing [94 - 97%]
round(mid_perc_time[4:7], 2) # decreasing [88 - 96%]
...
ggsave(plot = all_plots, "supplementary_figures/tv_rt_gradual_midwindow.pdf",
