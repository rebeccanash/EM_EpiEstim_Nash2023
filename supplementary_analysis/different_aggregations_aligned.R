## Different temporal aggregations - time windows align with step change
...
# t_start and t_end for each aggregation considered to ALIGN with step change
time <- list()
time[[1]] <- c(3, 68) # 3 day
time[[2]] <- c(1, 70) # 7 day
time[[3]] <- c(6, 65) # 10 day
time[[4]] <- c(8, 63) # 14 day

# 3-day aggregations aligned
start <- time[[1]][1]
end <- time[[1]][2]
...
# 
# list_vec <- setNames(vector("list", n_results), paste0("aligned_rt_3day_",
#                                                        1:n_results))
...
#                                  config = config)
#     assign(paste0("aligned_rt_3day_", res), rt_result)
#   }
# }
# 
# # Save result
# for(x in seq_along(names)) {
#   saveRDS(get(paste0("aligned_rt_3day_", x)),
#           file = paste0("supplementary_analysis/different_aggregations/result_rt_",
#                         names(sims_agg_3[x]), "_3day_aligned.rds"))
# }
# 
## Weekly sliding estimates for 10-day
# 
# list_vec <- setNames(vector("list", n_results), paste0("aligned_rt_10day_",
#                                                        1:n_results))
# list2env(list_vec, .GlobalEnv)
# rt_result <- list()
# 
...
#                                  method = "parametric_si",
#                                  config = config)
#     assign(paste0("aligned_rt_10day_", res), rt_result)
#   }
# }
# 
# # Save result
# for(x in seq_along(names)) {
#   saveRDS(get(paste0("aligned_rt_10day_", x)),
#           file = paste0("supplementary_analysis/different_aggregations/result_rt_",
#                         names(sims_agg_10[x]), "_10day_aligned.rds"))
# }
# 
# # Weekly sliding estimates for 14-day
# 
# list_vec <- setNames(vector("list", n_results), paste0("aligned_rt_14day_",
#                                                        1:n_results))
# list2env(list_vec, .GlobalEnv)
# rt_result <- list()
# 
...
#                                  method = "parametric_si",
#                                  config = config)
#     assign(paste0("aligned_rt_14day_", res), rt_result)
#   }
# }
# 
# # Save result
# for(x in seq_along(names)) {
#   saveRDS(get(paste0("aligned_rt_14day_", x)),
#           file = paste0("supplementary_analysis/different_aggregations/result_rt_",
#                         names(sims_agg_14[x]), "_14day_aligned.rds"))
# }
# 
# 
# # Match sliding estimates time window to 10-day aggregations
# 
# list_vec <- setNames(vector("list", n_results), paste0("aligned_rt_10day_match_",
#                                                        1:n_results))
# list2env(list_vec, .GlobalEnv)
# rt_result <- list()
# 
...
#                                  method = "parametric_si",
#                                  config = config)
#     assign(paste0("aligned_rt_10day_match_", res), rt_result)
#   }
# }
# 
# # Save result
# for(x in seq_along(names)) {
#   saveRDS(get(paste0("aligned_rt_10day_match_", x)),
#           file = paste0("supplementary_analysis/different_aggregations/result_rt_",
#                         names(sims_agg_10[x]), "_10day_aligned_match.rds"))
# }
# 
# # Match sliding estimates time window to 14-day aggregations
# 
# list_vec <- setNames(vector("list", n_results), paste0("aligned_rt_14day_match_",
#                                                        1:n_results))
# list2env(list_vec, .GlobalEnv)
# rt_result <- list()
# 
...
#                                  method = "parametric_si",
#                                  config = config)
#     assign(paste0("aligned_rt_14day_match_", res), rt_result)
#   }
# }
# 
# # Save result
# for(x in seq_along(names)) {
#   saveRDS(get(paste0("aligned_rt_14day_match_", x)),
#           file = paste0("supplementary_analysis/different_aggregations/result_rt_",
#                         names(sims_agg_14[x]), "_14day_aligned_match.rds"))
# }
...
for (n in seq_along(names)) {
  result3[[n]] <- readRDS(
    paste0("supplementary_analysis/different_aggregations/result_rt_",
           names[n], "_3day_aligned.rds"))
}

# 7 day weekly sliding results already aligned:
result7[[1]] <- readRDS("supplementary_analysis/constant_rt/result_rt_1.5.rds")
result7[[2]] <- readRDS("supplementary_analysis/tv_rt_sudden/result_rt_sud_1.5to1.25.rds")
result7[[3]] <- readRDS("supplementary_analysis/tv_rt_gradual/result_rt_grad_1.25to1.5.rds")

# 10 day
for (n in seq_along(names)) {
  result10[[n]] <- readRDS(
    paste0("supplementary_analysis/different_aggregations/result_rt_",
           names[n], "_10day_aligned.rds"))
}

for (n in seq_along(names)) {
  result10_match[[n]] <- readRDS(
    paste0("supplementary_analysis/different_aggregations/result_rt_",
           names[n], "_10day_aligned_match.rds"))
}
...
for (n in seq_along(names)) {
  result14[[n]] <- readRDS(
    paste0("supplementary_analysis/different_aggregations/result_rt_",
           names[n], "_14day_aligned.rds"))
}

for (n in seq_along(names)) {
  result14_match[[n]] <- readRDS(
    paste0("supplementary_analysis/different_aggregations/result_rt_",
           names[n], "_14day_aligned_match.rds"))
}
...
# adjust t_end so match aligning
# 3-day (removed first two days from inc)
for (i in 1:3) {
...
# 10-day (removed first five days from inc)
for (i in 7:9) {
  adj <- get(paste0("dat", i)) %>% 
    mutate(t_end = ifelse(source == "weekly" | source == "weekly_match",
                          t_end + 5, t_end))
  assign(paste0("dat", i), adj)
}

# 14-day (removed first 7 days from inc)
for (i in 10:12) {
  adj <- get(paste0("dat", i)) %>% 
    mutate(t_end = ifelse(source == "weekly" | source == "weekly_match",
                          t_end + 7, t_end))
...
# grey polygons to show which times were not included to align time windows
excl_b <- c(1:3, rev(1:3))
excl_e <- c(68:70, rev(68:70))
excl_y <- c(c(rep(0.95, 3)), c(rep(2.25, 3)))
excl_3 <- data.frame(excl_b, excl_e, excl_y)

excl_b <- c(1:6, rev(1:6))
excl_e <- c(65:70, rev(65:70))
excl_y <- c(c(rep(0.95, 6)), c(rep(2.25, 6)))
excl_10 <- data.frame(excl_b, excl_e, excl_y)

excl_b <- c(1:8, rev(1:8))
excl_e <- c(63:70, rev(63:70))
excl_y <- c(c(rep(0.95, 8)), c(rep(2.25, 8)))
excl_14 <- data.frame(excl_b, excl_e, excl_y)

# for dashed aggregation lines
guide_3 <- seq(5, 68, 3)
guide_7 <- seq(7, 70, 7)
guide_10 <- seq(15, 65, 10)
guide_14 <- seq(21, 63, 14)
...
for (p in seq_along(no_match)) {
    ...
    if (p <= 3) { 
      plot <- plot +
      geom_polygon(data = get(paste0("excl_", rep_agg[p])), aes(y = excl_y, x = excl_b),
                   fill = "grey96", alpha = 1, inherit.aes = FALSE) +
      geom_polygon(data = get(paste0("excl_", rep_agg[p])), aes(y = excl_y, x = excl_e),
                   fill = "grey96", alpha = 1, inherit.aes = FALSE)
    }
...
for (p in seq_along(match)) {
  plot <- r_plot_match(data = get(paste0("dat", p + 6)),
    ...
    with_exclude <- plot +
      geom_polygon(data = get(paste0("excl_", rep_agg_match[p])), aes(y = excl_y, x = excl_b),
                   fill = "grey96", alpha = 1, inherit.aes = FALSE) +
      geom_polygon(data = get(paste0("excl_", rep_agg_match[p])), aes(y = excl_y, x = excl_e),
                   fill = "grey96", alpha = 1, inherit.aes = FALSE)
    plot <- with_exclude
...
plot_r7 <- r_plot_match(data = dat7,
                          ymin = 1, ymax = 2,
                          legend_pos = c(0.67, 0.87),
                          agg_guide = guide_10,
                          axis_title = "Sliding R estimate",
                          legend_label = "10 day window",
                          hline = NULL)
plot_r7 <- plot_r7 + geom_polygon(data = excl_10, aes(y = excl_y, x = excl_b),
               fill = "grey96", alpha = 1, inherit.aes = FALSE) +
  geom_polygon(data = excl_10, aes(y = excl_y, x = excl_e),
               fill = "grey96", alpha = 1, inherit.aes = FALSE)
...
plot_r10 <- r_plot_match(data = dat10,
                     ...
                     hline = NULL)
plot_r10 <- plot_r10 + 
  geom_polygon(data = excl_14, aes(y = excl_y, x = excl_b),
               fill = "grey96", alpha = 1, inherit.aes = FALSE) +
  geom_polygon(data = excl_14, aes(y = excl_y, x = excl_e),
               fill = "grey96", alpha = 1, inherit.aes = FALSE)
...
ggsave(plot = all,"supplementary_figures/different_aggregations_aligned.pdf",
